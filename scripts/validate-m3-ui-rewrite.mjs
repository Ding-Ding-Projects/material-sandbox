...
const hasFreshPageHostBuild = source => {
  ...
  const qmakeExitIndex = source.indexOf('IF %ERRORLEVEL% NEQ 0 goto :page_host_tests_error', qmakeIndex);
  const launchIndex = source.indexOf(pageHostLaunch, qmakeIndex);
  const errorLabelIsDefined = /^:page_host_tests_error\r?$/m.test(source);
...
expect(!hasFreshPageHostBuild(qmakeBuild.replace(/^:page_host_tests_error\r?$/m, ':missing_page_host_tests_error')),
