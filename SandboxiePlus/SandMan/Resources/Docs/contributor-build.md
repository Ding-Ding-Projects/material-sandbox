...
Contributor builds normalize the driver capability state to the highest feature profile and suppress supporter, purchase, expiry, and certificate reminder interruptions. Settings keeps the updater child tab (renamed **Updates**) while removing the certificate child tab. Setup Wizard hides the commercial/personal license choice, clears stale usage markers, and routes directly to UI setup. Legacy support-dialog callers receive an allowed no-op result, so a silent reminder cannot terminate the manager.
