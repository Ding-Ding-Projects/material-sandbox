	...
	: CConfigDialog(parent)
{
	...
	if (ui.tabsTemplates && ui.tabAppCompat) {
		...
		if (compatIndex >= 0) {
			...
			auto* compatActions = new QVBoxLayout();
			ui.btnAddCompat = new QPushButton(tr("Enable"), nativeCompat);
			ui.btnDelCompat = new QPushButton(tr("Disable"), nativeCompat);
			ui.btnAddCompat->setAccessibleName(tr("Enable compatibility for selected application"));
			ui.btnDelCompat->setAccessibleName(tr("Disable compatibility for selected application"));
			auto* compatButtons = new QHBoxLayout();
			compatButtons->addWidget(ui.btnAddCompat);
			compatButtons->addWidget(ui.btnDelCompat);
			compatButtons->addStretch();
			compatActions->addLayout(compatButtons);
			...
			ui.lblUpdateTemplates->setWordWrap(true);
