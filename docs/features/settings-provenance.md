...
The provenance indicator is descriptive, not a second configuration store: it checks the existing `CSettings::ListSettings()` keys and never writes or exports a new marker. A profile value means the profile contains that key; a compiled-in value means the application is using its documented shipped value because the key is absent. This distinction survives restart and remains local to the current profile.

Unsupported typography properties remain disclosed by the native appearance editor; the provenance line does not claim that unsupported Word-style properties are implemented. No credential, token, or secret is read while evaluating provenance.
